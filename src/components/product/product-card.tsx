...
import { Heart, Plus, Clock, Truck } from "lucide-react"
...
export function ProductCard({
  ...
  return (
    ...
    >
      <Link href={`/modely/${product.slug}`} className="block">
        <div
          className={cn(
        ...
        >
          ...
          <button
            ...
            className="absolute right-2.5 top-2.5 flex size-8 items-center justify-center rounded-full bg-background/80 text-foreground backdrop-blur-sm transition-transform hover:scale-110"
          >
            <Heart
              className={cn(
          ...
          {!compact && (
            <div className="absolute inset-x-2.5 bottom-2.5 translate-y-3 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
              <button
                onClick={quickAdd}
                className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-background/95 py-2.5 text-xs font-semibold text-foreground backdrop-blur-sm hover:bg-brand-primary hover:text-brand-primary-foreground"
              >
